...
intro
- 인사말
0점: 첫인사 + 마무리 멘트 모두 수행 (첫인사 예: "안녕하세요", "감사합니다 OO카드입니다", "네 고객님" 등 상담 시작 시 인사. 마무리 멘트 예: "감사합니다", "좋은 하루 되세요", "또 문의주세요" 등 상담 종료 시 인사)
-5점: 첫인사와 마무리 멘트 모두 누락한 경우에만 감점. 둘 중 하나라도 수행했으면 0점
- 고객확인
0점: 고객정보를 고객에게 직접 질문
...
{{
    ...
    "category_raw": "아래 세부 카테고리 목록 중 가장 적합한 1개 택일",
    "inquiry": "고객이 문의한 핵심 내용을 1줄로 요약",
    "process": ["상담 과정 요약", "1단계", "2단계", ...],
    "result": "아래 형식의 구조화된 상세 요약:\\n\\n1줄 요약문\\n\\n[처리 내역]\\n1. 항목: 세부내용\\n\\n[고객 요청사항]\\n- 항목\\n\\n[상담사 조치]\\n- 항목\\n\\n[참고사항]\\n- 해당 시 기재 (없으면 생략)",
...
### 세부 카테고리 목록 (category_raw용, 49개)
가상계좌 안내 | 가상계좌 예약/취소 | 갱신 수령지 변경 | 결제계좌 안내/변경 | 결제대금 안내 | 결제일 안내/변경 | 결제일 안내/변경/취소 | 교육비 | 금리인하요구권 안내/신청 | 긴급 배송 신청 | 단기카드대출 안내/실행 | 도난/분실 신청/해제 | 도시가스 | 매출구분 변경 | 명세서 재발송 | 서비스 이용방법 안내 | 선결제/즉시출금 | 소득공제 확인서/종합소득세 안내 | 쇼핑케어 | 수기환급 처리 | 승인취소/매출취소 안내 | 신용공여기간 안내 | 심사 진행사항 안내 | 안심클릭/간편페이 안내 | 연체대금 안내 | 연체대금 즉시출금 | 연회비 안내 | 오토할부/오토캐쉬백 안내/신청/취소 | 옵션 서비스 안내/변경 | 이벤트 안내 | 이용내역 안내 | 이용방법 안내 | 일부결제대금이월약정 등록 | 일부결제대금이월약정 안내 | 일부결제대금이월약정 해지 | 입금내역 안내 | 장기카드대출 실행 | 장기카드대출 안내 | 전화요금 | 정부지원 바우처 | 증명서/확인서 발급 | 청구지 안내/변경 | 특별한도/임시한도/일시한도 안내/신청 | 포인트/마일리지 안내 | 포인트/마일리지 전환등록 | 프리미엄 바우처 안내/발급 | 한도 안내 | 한도상향 접수/처리 | 해외결제 문의
"""