...
**중요 규칙:**
- 금융 전문용어는 정확히 유지 (연회비, 바우처, 익일, 영업일 등)
- 카드상품명은 띄어쓰기 없이 한 단어로
- 문맥상 자연스러운 표현으로 교정
- 과도한 교정 금지 (의미가 명확하면 원문 유지)
"""
...
**출력 형식 (JSON만 출력, 다른 설명 금지):**
{
    ...
    "refined": "교정된 텍스트",
    "corrections": [
        {"from": "오류단어", "to": "교정단어", "reason": "이유"}
    ],
    "confidence": 0.95
}

**예시:**
입력: "연예비 납부와 그 바우저 한개 선택하라고"
출력:
{
    "original": "연예비 납부와 그 바우저 한개 선택하라고",
    "refined": "연회비 납부와 그 바우처 한 개 선택하라고",
    "corrections": [
        {"from": "연예비", "to": "연회비", "reason": "STT hallucination"},
        {"from": "바우저", "to": "바우처", "reason": "STT hallucination"}
    ],
    "confidence": 0.95
}
"""
