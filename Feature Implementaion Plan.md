...
`app/llm/education/tts_speaker.py`
- `persona_generator`에서 생성된 페르소나가 부여된 가상 고객과 사용자가 대화를 진행한다.
- 사용자 음성이 STT를 통해 텍스트로 변환된 것을 받아, 가상 고객이 응답한다. 이 응답은 TTS로 변환되어 재생된다.
- 개발 단계에서는 사용자 입력을 음성뿐 아니라 텍스트로 받을 수도 있도록 한다.
- 모델이 생성한 TTS 응답의 오디오 파일은 추후 테스트 용이를 위해 동일한 경로의 'tts_output' 폴더에 저장한다.
...
1. AI가 생성한 고객과 상담 시뮬레이션을 진행한다. 
2. 시뮬레이션은 음성으로 진행되며, 실제 상담과 동일하게 진행된다.
